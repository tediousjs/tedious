...
class Connection extends EventEmitter {
  constructor(config) {
    ...
    this.STATE = {
      ...
      SENT_PRELOGIN: new SentPreloginState(this),
      REROUTING: new ReRoutingState(this),
      TRANSIENT_FAILURE_RETRY: new TransientFailureRetryState(this),
      ...
      SENT_CLIENT_REQUEST: new SentClientRequestState(this),
      SENT_ATTENTION: new SentAttentionState(this),
...
class State {
  constructor(connection, { name, enter, events, exit }) {
...
class SentPreloginState extends State {
  constructor(connection) {
    super(connection, {
      name: 'SentPrelogin',
      events: {
        ...
        message: function(message) {
          message.pipe(new BufferList((err, data) => {
            const preloginPayload = new PreloginPayload(data);
            this.debug.payload(function() {
...
class ReRoutingState extends State {
  constructor(connection) {
    super(connection, {
      name: 'ReRouting',
      events: {
  ...
  enter() {
    this.connection.cleanupConnection(this.cleanupTypeEnum.REDIRECT);
  }
}

class TransientFailureRetryState extends State {
  constructor(connection) {
    super(connection, {
...
class SentTLSSSLNegotiationState extends State {
  constructor(connection) {
    super(connection, {
      ...
      events: {
        ...
        message: function(message) {
          message.pipe(this.messageIo.securePair.encrypted, { end: false });
...
class SentLogin7WithStandardLoginState extends State {
  constructor(connection) {
    super(connection, {
      ...
      events: {
        ...
        message: function(message) {
          let featureExtAckToken;
          const onFeatureExtAck = (token) => {
            featureExtAckToken = token;
          };
...
class SentLogin7WithNTLMLoginState extends State {
  constructor(connection) {
    super(connection, {
...
class SentLogin7WithFedauthState extends State {
  constructor(connection) {
    super(connection, {
      ...
      events: {
        ...
        message: function(message) {
          let fedAuthInfoToken;

          const onFedAuthInfo = (token) => {
            fedAuthInfoToken = token;
          };

          this.tokenStreamParser.on('fedAuthInfo', onFedAuthInfo);

          message.pipe(this.tokenStreamParser.parser, { end: false });

          message.on('end', () => {
            this.tokenStreamParser.removeListener('fedAuthInfo', onFedAuthInfo);
...
class LoggedInSendingInitialSqlState extends State {
  constructor(connection) {
    super(connection, {
...
class SentClientRequestState extends State {
  constructor(connection) {
    super(connection, {
      ...
      events: {
        ...
        message: function(message) {
          ...
          const onPause = () => {
            message.unpipe(this.tokenStreamParser.parser);
          ...
          this.request.on('resume', onResume);
          this.request.on('cancel', () => {
            if (this.request.isPaused()) {
              onResume();
            }
          });
          ...
          message.on('end', () => {
            // Make sure we haven't moved to another state
            if (this.state === this.STATE.SENT_CLIENT_REQUEST) {
              this.transitionTo(this.STATE.LOGGED_IN);
...
class SentAttentionState extends State {
  constructor(connection) {
    super(connection, {
      name: 'SentAttention',
      events: {
        ...
        message: function(message) {
          let attentionReceived = false;
          const onDoneToken = (token) => {
            if (token.attention) {
              attentionReceived = true;
            }
          };

          this.tokenStreamParser.on('done', onDoneToken);

          message.pipe(this.tokenStreamParser.parser, { end: false });

          message.on('end', () => {
            this.tokenStreamParser.removeListener('done', onDoneToken);
