...
var Connection, Request, TEST_SETUP_1, TEST_SETUP_2, TEST_SETUP_3, TYPES, config, fs, getConfig;
...
exports.callProcedureWithTVP = function(test) {
  ...
  test.expect(13);
  config = getConfig();
  request = new Request(TEST_SETUP_1, function(err, rowCount) {
    return connection.execSqlBatch(request2);
  });
  request2 = new Request(TEST_SETUP_2, function(err, rowCount) {
    return connection.execSqlBatch(request3);
  });
  request3 = new Request(TEST_SETUP_3, function(err, rowCount) {
    return connection.callProcedure(request4);
  });
  request4 = new Request('__tediousTvpTest', function(err, rowCount) {
  ...
  table = {
    ...
    rows: [[false, 1, 2, 3, 4, 5.5, 6.6, 'asdf', 'asdf', new Date(Date.UTC(2014, 0, 1))]]
  };
