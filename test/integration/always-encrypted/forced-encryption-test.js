...
describe('forced encryption test', function() {
  ...
  it('should test Numeric Prepared Statement', function(done) {
    ...
    forceEncryptTest([], call_sql, values, expected, array1, false, null, unencTable, addParamsArray, done, () => {
      forceEncryptTest([], call_sql, values, expected, array2, true, expErrMsg, unencTable, addParamsArray, done, () => {
        forceEncryptTest([], call_sql, values, expected, array1, false, null, encTable, addParamsArray, done, () => {
          forceEncryptTest([], call_sql, values, expected, array3, false, null, encTable, addParamsArray, done, done);
        });
      });
    });
  });
  ...
  xit('should test time prepared statement', function(done) {
    const encTable = encryptedTime;

    const call_sql_enc = "insert into [dbo].[" + encTable + "] values(@p1, @p1, @p2, @p3)";

    const dateObj = new Date(2015, 5, 18, 23, 59, 59);
    const dateTimeOffsetObj = new Date(Date.UTC(2014, 1, 14, 17, 59, 59, 999));

    const expDate = Buffer.from('03163a0b', 'hex');
    const expTime = 42171;
    const expDateTime2 = Buffer.from('067f5101163a0b', 'hex');
    const expDateTimeOffset = Buffer.from('0820fd002d380b', 'hex');

    const values = [dateObj, dateObj, dateObj, dateTimeOffsetObj];
    const expected = [expDate, expTime, expDateTime2, expDateTimeOffset];
