...
/**
 * @typedef {(hostname: string, options: dns.LookupAllOptions, callback: (err: NodeJS.ErrnoException | null, addresses: dns.LookupAddress[]) => void) => void} LookupFunction
 */

describe('Sender to IPv4 address', function() {
  /**
   * @type dgram.Socket
   */
  let server;
  ...
  describe('when given a custom DNS lookup function', function() {
    it('calls the custom DNS lookup function to resolve hostnames', function(done) {
      /**
       * @type {LookupFunction}
       */
      function customLookup(hostname, options, callback) {
        assert.deepEqual(hostname, 'localhost');
        assert.deepEqual(options, { all: true });

        callback(undefined, [{ address: server.address().address, family: server.address().family }]);
      }

      const lookupSpy = sinon.spy(customLookup);

      const sender = new Sender('localhost', server.address().port, lookupSpy, Buffer.from([0x02]));

      server.on('message', (msg, rinfo) => {
        sinon.assert.calledOnce(lookupSpy);
    ...
    it('passes through any errors returned by the custom DNS lookup function', function(done) {
      const lookupError = new Error('custom lookup failed');
      ...
      sender.execute((err) => {
        assert.strictEqual(err, lookupError);
  ...
  it('passes the returned message to the given callback', function(done) {
    /**
     * @type {AbortController}
     */
    const controller = new AbortController();
    const sender = new Sender(server.address().address, server.address().port, dns.lookup, Buffer.from([0x02]), controller.signal);
  ...
  it('does not call the given callback if aborted via `.cancel`', function(done) {
    /**
     * @type {AbortController}
     */
    const controller = new AbortController();
    ...
    sender.execute(() => {
      done(new Error('unexpected callback execution'));
    });
  });

  it('does not call the given callback if the given signal is aborted', function(done) {
    /**
     * @type {AbortController}
     */
    const controller = new AbortController();
