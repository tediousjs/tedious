...
upperGuids = lowerGuids.map((g) => g.toUpperCase())

mixedGuids = lowerGuids.map((g) => g.substr(0, 10) + g.substr(10).toUpperCase())
...
exports.guidToArray = (test) ->
  for guid, index in lowerGuids
    test.deepEqual(guidParser.guidToArray(guid), arrays[index])

  for guid, index in upperGuids
    test.deepEqual(guidParser.guidToArray(guid), arrays[index])

  for guid, index in mixedGuids
    test.deepEqual(guidParser.guidToArray(guid), arrays[index])
...
exports.arrayToGuid = (test) ->
  for array, index in arrays
    test.strictEqual(guidParser.arrayToGuid(array), upperGuids[index])
