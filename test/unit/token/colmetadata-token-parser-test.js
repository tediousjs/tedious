...
describe('Colmetadata Token Parser', () => {
  ...
  it('should parse crypto-metadata', () => {
    ...
    const columnCryptoEncryptionKeys = columnCryptoMetadata.encryptionKeys;

    assert.strictEqual(columnCryptoEncryptionKeys.length, 0x01);
    const columnEncryptionKeyValue = columnCryptoEncryptionKeys[0];
    assert.isNotNull(columnEncryptionKeyValue);
    assert.strictEqual(columnEncryptionKeyValue.databaseId, 0x05);
