var NTLMPayload = require('../../src/ntlm-payload');
...
exports.respondToChallenge = function(test) {
  ...
  var response = new NTLMPayload(challenge);
  ...
  var domainName = response.data.slice(64, 76).toString('ucs2');
  var userName = response.data.slice(76, 92).toString('ucs2');
  var targetData = response.data.slice(160, 164).toString('hex');
